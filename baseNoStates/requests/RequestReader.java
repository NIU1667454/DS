...
public class RequestReader implements Request {
  ...
  // comprova si l'usuari pot accedir a aquesta hora
  private boolean checkTime(UserGroup user) {
    Timetable table = user.getSchedule();
    // hora del moment en el que s'intenta obrir la porta
    LocalTime time = LocalTime.of(now.getHour(), now.getMinute(), now.getSecond());

    //es comprova si l'hora està entre les permeses
  ...
  // comprova si l'usuari pot accedir aquell dia de la setmana i aquella data
  private boolean checkDate(UserGroup user) {
    Timetable table = user.getSchedule();
    LocalDate today = LocalDate.of(now.getYear(), now.getMonthValue(), now.getDayOfMonth());
    // primer mirem si la data es troba entre els valors permesos
    if (!today.isBefore(table.getStartDate()) && !today.isAfter(table.getEndDate())) {
      ...
      // si el dia actual es troba a la llista de dies que pot accedir, autoritzem l'operació
  ...
  // s'encarrega de comprovar si l'usuari pot tenir accés a aquesta àrea
  private boolean checkArea(UserGroup user, Door door) {
    Area a = door.getToSpace(); // obtenim l'àrea on s'intenta obrir la porta

    // iterem els pares de cada node fins a trobar un que estigui
    // dintre de les areas permeses o el pare sigui null
    ...
    // en cas de poder gestionar totes les areas només contindrà la cadena building (arrel)
    if (user.getAreas().contains(a)) {
      return true;
    }
